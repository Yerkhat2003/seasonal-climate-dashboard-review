...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="cluster-overview panel">
      ...
      <div className="timeline-caption"><span className="legend-warm" /> Тёплый сезон: средний период, когда недельная температура не ниже 15 °C</div>
    ...
    <section className="insights">
      ...
      <article className="panel insight-card"><div className="card-icon warm"><CalendarDays size={20} /></div><span className="section-kicker">ТЁПЛЫЙ СЕЗОН</span><h3>{transition?.springDay ? `≈ с ${Math.ceil(transition.springDay / 7)}-й недели` : 'Недостаточно данных'}</h3><p className="hover-tip transition-info" tabIndex={0} data-tooltip="Весенняя граница: средняя дата устойчивого перехода через 15 °C. Она появляется после двух последовательных недель не ниже этого порога и усредняется по историческим годам.">Весенний переход <Info size={12} /></p></article>
    ...
    <section className="faq panel" id="faq">
      ...
      <div className="faq-list">
        ...
        <details><summary>Что такое тёплый сезон?</summary><p>Это рабочая климатическая метка для сравнения городов: период между весенней и осенней границей 15 °C. Она не означает, что весь период каждый день тёплый, и не является календарным сезоном.</p></details>
        <details><summary>Как именно считаются границы тёплого сезона?</summary><p>Весенняя граница появляется после двух последовательных недель со средней температурой не ниже 15 °C. Осенняя граница определяется как последняя из двух последовательных тёплых недель. Затем даты усредняются по историческим годам. Такое правило снижает влияние одиночного краткого потепления или похолодания.</p></details>
