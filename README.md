...
Ежедневные многолетние ряды не сохраняются. При каждом обновлении генератор заново получает их из публичного API, пересчитывает недельные данные, климатологию, сезонные переходы и аналоги, затем публикует только результат. В коде нет захардкоженных погодных значений: фиксированы только координаты городов и параметры методики, например порог сезонного перехода 15 °C.
...
Кнопка **Excel** выгружает текущий выбранный срез в `.xlsx`: параметры фильтра, недельные значения на графике, сведения о городах и границах сезона, аналоги и результат их исторической проверки.
...
- Сезонные переходы: средняя неделя устойчивого перехода через 15 °C (неделя не ниже порога); в интерфейсе также передаётся межгодовой разброс.
