...
function transitions(records) {
  ...
  for (const year of byYear.values()) {
    ...
    const springWeek = weeks.find((week, index) => week.week >= 8 && week.week <= 30 && mean(week.mean) >= 5 && mean(weeks[index + 1]?.mean ?? []) >= 5)?.week
    const autumnWeek = [...weeks].reverse().find((week, reverseIndex, reversed) => week.week >= 28 && week.week <= 48 && mean(week.mean) >= 5 && mean(reversed[reverseIndex + 1]?.mean ?? []) >= 5)?.week
